  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 sm:gap-2">
        {depth > 0 && (
          <div className="text-text-muted shrink-0 flex items-center justify-center w-4">
            <CornerDownRight size={13} className="text-text-muted" />
          </div>
        )}
        <span className="text-[10px] font-mono text-text-muted min-w-[22px] text-right shrink-0">
          {pathNumber}
        </span>
        <input
          ref={inputRef}
          ...
          className="flex-1 min-w-0 px-3 py-2.5 rounded-[10px] bg-bg border border-line text-sm text-text placeholder-text-muted focus:outline-none focus:border-[var(--accent)]"
        />
        <button
          type="button"
          ...
          className="w-9 h-9 rounded-[10px] bg-surface-raised text-text-muted hover:text-[var(--accent)] hover:bg-[var(--accent-15)] flex items-center justify-center shrink-0 transition-colors"
        >
          <Plus size={15} />
        </button>
        <button
          ...
          className="w-9 h-9 rounded-[10px] bg-surface-raised text-text-muted hover:text-danger hover:bg-danger/10 flex items-center justify-center shrink-0 transition-colors"
        >
          <X size={15} />
        </button>
      </div>
      ...
      {tieneHijos && sub.subtareas && (
        <div className="ml-3 sm:ml-4 pl-2.5 sm:pl-3 border-l-2 border-line-strong/70 space-y-2">
...
const RootSubtareaRow: React.FC<{
  ...
  return (
    <Reorder.Item
      ...
      className="space-y-2 bg-[#12141A]/50 p-2.5 rounded-[14px] border border-line/80"
    >
      <div className="flex items-center gap-1.5 sm:gap-2">
        <button
          type="button"
          ...
          className="shrink-0 w-6 h-9 flex items-center justify-center text-text-muted hover:text-text-muted cursor-grab active:cursor-grabbing"
          style={{ touchAction: 'none' }}
        >
          <GripVertical size={16} />
        </button>
        <span className="text-[11px] font-mono font-bold text-text-muted w-4 text-center shrink-0">
          {index + 1}
        </span>
        <input
          type="text"
      ...
      {sub.subtareas && sub.subtareas.length > 0 && (
        <div className="ml-3 sm:ml-4 pl-2.5 sm:pl-3 border-l-2 border-line-strong/70 space-y-2 pt-1">
          {sub.subtareas.map((child, cIdx) => (
            <SubtareaEditorNode
              key={child.id}
              sub={child}
...
export const TareaEditorModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-end sm:items-center justify-center bg-black/75 backdrop-blur-sm animate-fadeIn" onClick={closeTareaEditor}>
      <div className="w-full max-w-[460px] max-h-[88vh] bg-surface border-t sm:border border-line rounded-t-[24px] sm:rounded-[24px] flex flex-col shadow-2xl animate-slideUp overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <header className="px-5 py-4 border-b border-line flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-bold font-heading text-text">{isEditing ? 'Editar tarea' : 'Nueva tarea'}</h2>
          </div>
          <button type="button" onClick={closeTareaEditor} aria-label="Cerrar" className="w-8 h-8 rounded-full bg-surface-raised text-text-muted hover:text-text flex items-center justify-center"><X size={16} /></button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
          ...
          <div className="space-y-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-text-muted">Usa + para crear sub-pasos</span>
            </div>
            ...
            <button
              type="button"
              onClick={addRootSub}
              className="w-full py-2.5 rounded-[12px] border border-dashed border-line-strong text-text-muted hover:text-text hover:border-line-strong hover:bg-surface text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
            >
