  ...
  return (
    <Reorder.Item value={habito.id} dragListener={false} dragControls={controls} as="div"
      ...
      <button type="button" onPointerDown={(e) => controls.start(e)} aria-label="Arrastrar hábito"
        className="shrink-0 w-6 h-9 flex items-center justify-center text-[#5C6070] hover:text-[#9498A8] cursor-grab active:cursor-grabbing" style={{ touchAction: 'none' }}>
      ...
      <span className="text-xs font-semibold font-heading text-[#F4F4F6] truncate flex-1">{habito.nombre}</span>
      <button type="button" onClick={onRemove} aria-label="Quitar de la rutina"
        className="w-7 h-7 rounded-lg bg-[#1A1C24] text-[#6B6F7B] hover:text-[#F87171] flex items-center justify-center shrink-0"><X size={14} /></button>
    </Reorder.Item>
  );
};
...
export const RutinaEditorModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[55] flex items-end sm:items-center justify-center bg-black/75 backdrop-blur-sm animate-fadeIn" onClick={closeRutinaEditor}>
      <div className="w-full max-w-[430px] max-h-[88vh] bg-[#14161D] border-t sm:border border-[#1E2029] rounded-t-[24px] sm:rounded-[24px] flex flex-col shadow-2xl animate-slideUp overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <header className="px-5 py-4 border-b border-[#1E2029] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-base font-bold font-heading text-[#F4F4F6]">{isEditing ? 'Editar rutina' : 'Nueva rutina'}</h2>
          </div>
          <button type="button" onClick={closeRutinaEditor} aria-label="Cerrar" className="w-8 h-8 rounded-full bg-[#1A1C24] text-[#9498A8] hover:text-white flex items-center justify-center"><X size={16} /></button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
          ...
          <div className="space-y-3">
            <label className="text-xs font-medium text-[#9498A8]">Hábitos de esta rutina ({seleccion.length})</label>

            {seleccion.length > 0 && (
              <div className="space-y-2">
                <p className="text-[11px] text-[#6B6F7B]">Arrastra desde el asa para definir el orden del enfoque</p>
            ...
            {habitosActivos.length === 0 ? (
              <p className="text-xs text-[#6B6F7B] py-4 text-center">Primero crea algunos hábitos.</p>
            ) : (
              habitosActivos.filter((h) => !seleccion.includes(h.id)).length > 0 && (
                <div className="space-y-2">
                  <p className="text-[11px] text-[#6B6F7B]">Añadir hábitos</p>
                  {habitosActivos.filter((h) => !seleccion.includes(h.id)).map((h) => (
                    <button key={h.id} type="button" onClick={() => toggleHabito(h.id)}
                      className="w-full p-3 rounded-[14px] border bg-[#0C0D12] border-[#1E2029] hover:border-[#2D313F] flex items-center gap-3 text-left transition-all">
                      ...
                      <span className="text-xs font-semibold font-heading text-[#F4F4F6] truncate flex-1">{h.nombre}</span>
                      <div className="w-5 h-5 rounded-md flex items-center justify-center shrink-0 border-2 border-[#2D313F]"><Plus size={12} className="text-[#6B6F7B]" /></div>
                    </button>
                  ))}
                </div>
              )
