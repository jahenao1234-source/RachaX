...
export const CreateHabitScreen: React.FC<CreateHabitScreenProps> = ({ habitToEdit: propHabitToEdit, onClose }) => {
  ...
  return (
    <div
      ...
      className="min-h-full bg-[#0C0D12] text-[#F4F4F6] flex flex-col animate-slideUp pb-28"
    >
      {/* Top Header */}
      <header className="sticky top-0 z-50 bg-[#0C0D12]/95 backdrop-blur-md border-b border-[#1E2029] px-4 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="w-9 h-9 rounded-full bg-[#14161D] hover:bg-[#1A1C24] border border-[#1E2029] text-[#6B6F7B] hover:text-[#F4F4F6] flex items-center justify-center transition-colors active:scale-95"
          >
            <X size={18} />
          </button>
          <div className="flex items-center gap-2">
            {isEditing && <Edit3 size={17} className="text-[var(--accent)]" />}
            <h1 className="text-lg font-bold font-heading text-[#F4F4F6]">
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-6 flex-1 max-w-[430px] mx-auto w-full">
        {!isEditing && (
          <div className="space-y-2">
            <label className="text-xs font-medium text-[#9498A8] flex items-center gap-1.5">
              <Sparkles size={14} className="text-[var(--accent)]" />
              Plantillas rápidas <span className="text-[11px] text-[#6B6F7B]">(opcional)</span>
            </label>
            <div className="flex items-center gap-2 overflow-x-auto pb-1.5 no-scrollbar -mx-1 px-1">
              {PLANTILLAS_HABITOS.map((p) => (
                <button
                  ...
                  className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-[12px] text-xs font-medium border whitespace-nowrap transition-all bg-[#14161D] border-[#1E2029] text-[#9498A8] hover:text-[#F4F4F6] hover:border-[#2D313F] active:scale-95"
                >
            ...
            <p className="text-[10px] text-[#6B6F7B] pl-1">Toca una para rellenar el formulario; luego ajústala y guarda.</p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <button
              ...
              className={`p-3 rounded-[14px] border text-left transition-all flex items-center gap-3 ${
                tipo === 'positivo'
                  ? 'bg-[var(--accent-15)] border-[var(--accent)] text-[#F4F4F6] shadow-sm'
                  : 'bg-[#14161D] border-[#1E2029] text-[#6B6F7B] hover:text-[#9498A8]'
              }`}
            >
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                  tipo === 'positivo'
                    ...
                    : 'bg-[#1A1C24] text-[#6B6F7B]'
                }`}
              >
                <CheckCircle2 size={18} />
              </div>
              <div className="min-w-0">
                <span className={`text-xs font-bold font-heading block ${tipo === 'positivo' ? 'text-[#F4F4F6]' : 'text-[#9498A8]'}`}>
                  Quiero hacer
                </span>
                <span className="text-[10px] text-[#6B6F7B] block truncate">
                  Construir hábito
                </span>
              </div>
            </button>
            ...
            <button
              ...
              className={`p-3 rounded-[14px] border text-left transition-all flex items-center gap-3 ${
                tipo === 'negativo'
                  ? 'bg-[#1E2029] border-[#5C6070] text-[#F4F4F6] shadow-sm'
                  : 'bg-[#14161D] border-[#1E2029] text-[#6B6F7B] hover:text-[#9498A8]'
              }`}
            >
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                  tipo === 'negativo'
                    ? 'bg-[#5C6070] text-white shadow-md shadow-black/40'
                    : 'bg-[#1A1C24] text-[#6B6F7B]'
                }`}
              >
                <ShieldCheck size={18} />
              ...
              <div className="min-w-0">
                <span className={`text-xs font-bold font-heading block ${tipo === 'negativo' ? 'text-[#F4F4F6]' : 'text-[#9498A8]'}`}>
                  Quiero evitar
                </span>
                <span className="text-[10px] text-[#6B6F7B] block truncate">
                  Romper mal hábito
        ...
        <div className="space-y-1.5">
          <label htmlFor="habit-name-input" className="text-xs font-medium text-[#9498A8] flex items-center gap-1.5">
            <Tag size={14} className="text-[var(--accent)]" />
            Nombre del Hábito <span className="text-[#F87171]">*</span>
          </label>
          <input
            id="habit-name-input"
            type="text"
            ...
            maxLength={60}
            autoFocus={!isEditing}
            className={`w-full px-4 py-3 rounded-[12px] bg-[#14161D] border text-sm text-[#F4F4F6] placeholder-[#5C6070] focus:outline-none transition-all ${
              errorNombre
                ? 'border-[#F87171] focus:ring-1 focus:ring-[#F87171]'
                : 'border-[#1E2029] focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]'
            }`}
          />
          {errorNombre && (
            <p id="habit-name-error" className="text-xs text-[#F87171] font-medium pl-1 animate-fadeIn">
              {errorNombre}
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[#9498A8] flex items-center gap-1.5">
              <Tag size={14} className="text-[var(--accent)]" />
              Categoría <span className="text-[11px] text-[#6B6F7B]">(opcional)</span>
            </label>
            {categoria && (
              <button
                type="button"
                onClick={() => setCategoria(undefined)}
                className="text-[11px] text-[#6B6F7B] hover:text-[#9498A8] transition-colors"
              >
                Quitar
              </button>
            )}
          ...
          <div className="grid grid-cols-6 gap-2">
            {COLORES_HABITO.map((c) => {
              ...
              return (
                ...
                >
                  {isSelected && <Check size={18} className="text-white drop-shadow" strokeWidth={3} />}
                </button>
              );
            })}
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-[16px] bg-[#14161D] border border-[#1E2029] p-3 max-h-[260px] overflow-y-auto space-y-3.5 divide-y divide-[#1E2029]/40">
            {groupedIcons.map((group, groupIdx) => (
              <div key={group.category} className={groupIdx > 0 ? 'pt-3 space-y-1.5' : 'space-y-1.5'}>
                <p className="text-[11px] font-semibold text-[#6B6F7B] px-0.5 tracking-wide">
                  {group.category}
                </p>
                      ...
                      <button
                        ...
                        className={`aspect-square rounded-[10px] flex items-center justify-center transition-all ${
                          isSelected
                            ...
                            : 'text-[#6B6F7B] hover:text-[#F4F4F6] hover:bg-[#1A1C24]'
                        }`}
                      >
                ...
                <button
                  ...
                  className={`py-2.5 px-2 text-center rounded-[12px] text-xs font-medium border transition-all flex items-center justify-center gap-1.5 ${
                    isSelected
                      ...
                      : 'bg-[#14161D] border-[#1E2029] text-[#6B6F7B] hover:text-[#F4F4F6]'
                  }`}
                >
        ...
        <div className="space-y-1.5">
          <label htmlFor="habit-anchor-input" className="text-xs font-medium text-[#9498A8] flex items-center gap-1.5">
            <Link2 size={14} className="text-[var(--accent)]" />
            Anclaje <span className="text-[11px] text-[#6B6F7B]">(opcional)</span>
          </label>
          <div className="flex items-center gap-2 rounded-[12px] bg-[#14161D] border border-[#1E2029] px-3 focus-within:border-[var(--accent)] transition-colors">
            <span className="text-xs text-[#6B6F7B] shrink-0 whitespace-nowrap">Después de</span>
            <input
              ...
              maxLength={50}
              className="flex-1 bg-transparent py-3 text-sm text-[#F4F4F6] placeholder-[#5C6070] focus:outline-none"
            />
          </div>
          <p className="text-[10px] text-[#6B6F7B] pl-1">Encadena el hábito a algo que ya haces (habit stacking).</p>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          {frecuencia === 'personalizado' && (
            <div className="p-3 rounded-[14px] bg-[#14161D] border border-[#1E2029] space-y-2 animate-fadeIn">
              <span className="text-[11px] text-[#6B6F7B] block">Selecciona los días:</span>
                    ...
                    <button
                      ...
                      className={`h-9 rounded-lg text-xs font-semibold transition-all flex items-center justify-center ${
                        isSelected
                          ...
                          : 'bg-[#1A1C24] text-[#6B6F7B] hover:text-[#F4F4F6]'
                      }`}
                    >
                      {d.label}
                    </button>
          ...
          {frecuencia === 'semanal' && (
            <div className="p-3 rounded-[14px] bg-[#14161D] border border-[#1E2029] flex items-center justify-between animate-fadeIn">
              <div>
                <span className="text-xs font-medium text-[#F4F4F6] block">Veces por semana</span>
                <span className="text-[10px] text-[#6B6F7B]">Cúmplelo los días que quieras</span>
              </div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setVecesPorSemana((v) => Math.max(1, v - 1))} className="w-8 h-8 rounded-full border border-[#2D313F] text-[#9498A8] hover:text-white flex items-center justify-center text-lg leading-none">−</button>
                <span className="w-8 text-center text-sm font-bold font-heading text-[var(--accent)]">{vecesPorSemana}</span>
                <button type="button" onClick={() => setVecesPorSemana((v) => Math.min(7, v + 1))} className="w-8 h-8 rounded-full bg-[var(--accent)] text-white flex items-center justify-center text-lg leading-none">+</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="rounded-[16px] bg-[#14161D] border border-[#1E2029] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <div className="p-2 rounded-xl bg-[#1A1C24] text-[var(--accent)]">
                <Target size={18} />
              </div>
              <div>
                <p className="text-xs font-medium text-[#F4F4F6]">Meta Cuantificable</p>
                <p className="text-[10px] text-[#6B6F7B]">Ej. 8 vasos, 20 páginas</p>
              </div>
            </div>
            <button
              type="button"
              ...
              className={`w-11 h-6 rounded-full transition-colors relative p-0.5 ${
                tieneMeta ? 'bg-[var(--accent)]' : 'bg-[#1E2029]'
              }`}
            >
              <div
          ...
          {tieneMeta && (
            <div className="pt-2 border-t border-[#1E2029] flex items-center gap-3 animate-fadeIn">
              <label htmlFor="target-number-input" className="text-xs text-[#9498A8] shrink-0">
                Objetivo por día:
              </label>
              <input
                ...
                className="w-24 px-3 py-1.5 rounded-lg bg-[#1A1C24] border border-[#1E2029] text-sm text-center text-[#F4F4F6] focus:outline-none focus:border-[var(--accent)]"
              />
            </div>
          )}
        </div>
        ...
        <div className="rounded-[16px] bg-[#14161D] border border-[#1E2029] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-xs font-medium text-[#F4F4F6]">Recordatorio Diario</p>
                <p className="text-[10px] text-[#6B6F7B]">Hora preferida para cumplir</p>
              </div>
            </div>
            <button
              type="button"
              ...
              className={`w-11 h-6 rounded-full transition-colors relative p-0.5 ${
                tieneRecordatorio ? 'bg-[var(--accent)]' : 'bg-[#1E2029]'
              }`}
            >
              <div
                className={`w-5 h-5 rounded-full bg-white transition-transform ${
          ...
          {tieneRecordatorio && (
            <div className="pt-2 border-t border-[#1E2029] flex items-center justify-between animate-fadeIn">
              <span className="text-xs text-[#9498A8]">Hora:</span>
              <input
                type="time"
                value={horaRecordatorio}
                onChange={(e) => setHoraRecordatorio(e.target.value)}
                className="px-3 py-1.5 rounded-lg bg-[#1A1C24] border border-[#1E2029] text-xs font-mono text-[#F4F4F6] focus:outline-none focus:border-[var(--accent)]"
              />
            </div>
          )}
        </div>
        ...
        <div className="pt-2">
          <button
            ...
            type="submit"
            className="w-full py-3.5 rounded-[12px] bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white font-heading font-bold text-sm shadow-lg shadow-[var(--accent-30)] transition-all active:scale-[0.98] flex items-center justify-center gap-2"
          >
