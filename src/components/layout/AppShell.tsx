...
export const AppShell: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#08080C] flex flex-col justify-center items-center py-0 sm:py-6 lg:py-8 px-0 sm:px-4 lg:px-6">
      {/* App Container: Phone format on mobile/tablet, wide workstation on desktop */}
      <main
        id="app-shell-main"
        className="w-full max-w-[430px] min-h-screen sm:min-h-[850px] sm:max-h-[920px] lg:max-w-[1200px] lg:h-[880px] lg:min-h-[860px] lg:max-h-[92vh] bg-[#0C0D12] sm:border sm:border-[#1E2029] sm:rounded-[36px] lg:rounded-[28px] relative flex flex-col lg:flex-row overflow-hidden sm:shadow-2xl sm:shadow-black/90"
      >
        {/* Ambient Top Glow */}
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden relative">
          ...
          <header className="px-4 py-2.5 flex items-center justify-between border-b border-[#1E2029]/50 relative z-10 shrink-0 lg:hidden">
            <button
              type="button"
              onClick={() => {
                closeHabitDetail();
            ...
            >
              <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[var(--accent)] to-[var(--accent-deep)] flex items-center justify-center text-white shadow-sm shadow-[var(--accent-30)] group-hover:scale-105 transition-transform">
                <Flame size={14} className="fill-white" />
              </div>
              <span className="font-heading font-bold text-sm text-[#F4F4F6] tracking-tight">
                Racha
              </span>
            </button>
